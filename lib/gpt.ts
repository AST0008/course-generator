...
export async function strict_output(
  ...
  model: string = "models/gemini-1.5-flash", // Default Gemini model
  ...
  for (let i = 0; i < num_tries; i++) {
    ...
    try {
      ...
      const completePrompt = `${completeSystemPrompt}\n\nUser: ${user_prompt.toString()}`;
      ...
      if (verbose) {
        console.log("System prompt:", completeSystemPrompt);
      ...
      try {
        let output: any = JSON.parse(jsonStr);
        ...
        console.log("Current invalid json format ", jsonStr);
      }
    } catch (apiError) {
      error_msg = `\n\nAPI Error: ${apiError}`;
